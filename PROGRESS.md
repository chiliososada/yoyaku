...
## 🧾 入力検証: 営業時間の重複・逆転を弾く（2026-07-30）
店主が「時間帯を追加」を押しすぎて **同じ曜日の同じ時間帯が2行** になっても保存できてしまった。
検証は最大50行しか見ておらず、重なりも `終了<=開始` も素通り。
- **症状（実測）**: `computeAvailability` は営業区間ごとに独立して候補時刻を生成して連結するため、
  同一区間が2つあると **顧客の予約画面に同じ時刻が2回並ぶ**（1区間9枠 → 2行で18枠・ユニークは9）。
  日付の○△×の空き数も水増しされ、React の `key={startAt}` も重複する。
  ※ 予約の二重確保は起きない（GiST排他制約と実占有で担保）。壊れるのは**表示**のみ。
- **もう1つ発見**: `終了<=開始`（例 月曜19:00-10:00）も保存でき、
  `resolveOpenIntervalsForDate` が `closeMinute > openMinute` で静かに除外するため、
  **その曜日が理由の説明もなく定休になる**（店主は原因に辿り着けない）。
- 修正2層: ①`businessHoursSchema` に superRefine で「終了<=開始」と「同一曜日の重なり」を拒否。
  エラー文に**曜日と実際の時刻**を出す（「月曜の時間帯が重なっています（10:00〜19:00 と 10:00〜19:00）」）。
  隣接（13:00終了→13:00開始）は重なり扱いしない＝連続営業として受理。
  ②`resolveOpenIntervalsForDate` で同一曜日の重なる区間を連結（検証前の既存データ・API直叩き対策）。
- テスト +11（連結5・検証8のうち新規分）→ **vitest 319 全緑**。

## 🩹 「ユーザーに疑問を残さない」一斉修正（2026-07-31・全14件）
営業時間に月曜が2行入る件（前節）をきっかけに、**同じ性質の欠陥＝画面には何も出ないのに
結果だけがおかしくなる**箇所を全系統で洗い出し（33エージェント監査で27件検出→対抗検証）、
実在する14件を8バッチで修正。全ゲート緑（tsc / lint / **vitest 367** / build）。

**入力を黙って捨てない・矛盾を保存させない**
- スタッフのシフトと営業時間で、フォームが不正行を `.filter()` で**捨ててから**送信していた
  （＝検証が一度も動かない）。フォーム・サービス層の両方で除去し、理由付きで拒否する。
- メニューの「時間帯分割」を検証（重なり・所要時間との不一致）。区間を1回足すだけで
  90分メニューが実質30分になり、**同じスタッフに30分間隔で予約が入る**状態だった。
- 予約ルールの組合せ検証: 締切が受付期間を超えると全日程が締切超過になり、
  画面上は「全部×」としか見えない（#22）。
- 店舗設定の「同時受付数」が SHOP 容量ルールに書き込まれず**保存しても効いていなかった**。

**「空きが無い」の理由を取り違えない**
- 担当必須なのに候補スタッフ0人の枠を「空きあり」と表示していた。客は氏名・連絡先まで
  入力し終えた**最後の確定ボタンで初めて**失敗する（しかも理由が出ない）→ 空き0＋STAFF_UNAVAILABLE。
- 誰も出勤していない枠の理由が SLOT_FULL（満席）だった。予約0件なのに「満席」と出るのが
  一番原因に辿り着けない → STAFF_UNAVAILABLE に修正（#1）。
- 「受付開始前」と「満席」を別の理由コードに分離（BEYOND_WINDOW）。
- スタッフの重複シフト区間を連結（営業時間側と同じ二重防御）。

**取り返しのつかない操作の前に知らせる（自動では消さない）**
- 臨時休業・スタッフ欠勤を登録した日に**既に予約が入っていたら件数を表示**。
  自動キャンセルはしない（客への連絡なしに来店を無にするため、判断は店主に委ねる）。
- 臨時営業日を足しても、シフトは曜日ベースなので**出勤者ゼロなら全枠が客に見えない**
  → 登録直後に警告（#1）。
- キャンセル期限が締切より長いと、短期の予約は**最初からキャンセル不可**になる。
  予約ルール画面で注意喚起し、`/booking/[token]` に**店舗の電話番号**を出して
  「店舗へご連絡ください」を行き止まりにしない（#9）。

**祝日を計算に変える（#13）**
- 祝日マスタは手入力の2026年分のみで、追加する画面も無かった。年が変われば
  「祝日は休業」設定が**何のエラーも出さずに効かなくなる**。
- `src/lib/jp-holidays.ts` で祝日法から算出（ハッピーマンデー・春分/秋分・振替休日の連鎖・
  国民の休日）。参照3箇所を差し替え、seed の投入を廃止。
  テストで 2026 年が従来の実データと完全一致、2027/2028 の春分・秋分が官報どおりであることを確認。

**ホームページ（#25 #26 #27）**
- 営業時間表と JSON-LD が祝日休業も臨時休業も無視 → 「※祝日は休業」表示、直近60日の
  お知らせ一覧、JSON-LD に PublicHolidays と specialOpeningHoursSpecification を追加。
- 「プレビュー」が未公開だと 404（理由の表示なし）→ `/admin/homepage/preview` を追加。
  ログイン必須・自テナントのみ・公開状態を問わず同じ見た目で描画し、上部に下書き帯を出す。
- ネット予約OFFのまま公開すると**予約ボタンの無いHP**が出来る → 編集画面で警告。

**据え置き（理由付き）**
- スタッフの「同時対応数」は GiST 排他制約（1スタッフ1予約）と両立しないため入力欄を
  読み取り専用の 1 に固定。保存できると「画面は空き、確定で失敗」を生むため。

## 📅 スケジュールボード: 週表示 + スタッフ絞り込み + 勤務時間の可視化（2026-07-31）
「店長はスタッフを指定してその人の予定を見られるか」という問いから。
できてはいたが、見られるのは `/admin/staff/[id]/schedule` の**設定値**（曜日シフト＋例外の一覧）だけで、
「来週この人は何日出勤するのか」は曜日シフト × カレンダー × 例外を頭で合成しないと分からなかった。
日別タイムラインには予約しか描かれず、勤務時間は出ていなかった。

- **週間シフト表**（縦: スタッフ / 横: 7日・月曜始まり）。セルは**実際の勤務時間**＝シフト ∩ 営業区間。
  予約件数バッジ付き。セルをクリックでその日の詳細へ。
- **日別タイムライン**に勤務時間を描画（勤務時間だけ白く抜き、勤務外は灰色）。
- **スタッフ絞り込み**（全員／個別）。日・週・日付送りのどのリンクでも絞り込みを保持。
- 休業の**理由**（定休日／祝日休業／臨時休業）と祝日名を表示。
- スタッフ一覧・スタッフ編集・シフト設定の各画面から「週間の予定」へ導線。

**実装方針**: 勤務時間は予約エンジンと**同じ純関数**（`resolveOpenIntervalsForDate` /
`resolveStaffWorkingIntervals`）で合成する。別ロジックで再実装すると画面の表示と実際の予約可否がずれ、
どちらも信じられなくなるため。読み込みは `loadRangeContext` 1回（週7日でもクエリ数は一定）。

**多視点レビュー（4観点19エージェント・対抗検証つき）で3件を修正**:
1. **退職・停止したスタッフの予約が board から消えていた**（4観点が独立に検出）。
   `softDeleteStaff` は既存予約を取り消しも付け替えもしないため「担当者が退職済みの予約」は普通に残るが、
   レーンを稼働スタッフだけで作っていたため**どのレーンにも入らず描画されない**。
   タイムラインの幅だけは広がるので、店主には「空欄が伸びている」としか見えず、客は当日ふつうに来店する。
   → 予約が参照するスタッフを引き当て、「退職・停止中」レーン／行として必ず描く（週の件数も同様）。
2. **休業日に予約が残っているとき、休業の理由が画面に出なかった**。
   この状態は意図的に `isClosed=false`（店主が対応できるよう閉じない）にしているが、
   ラベルを祝日名の有無で条件分岐していたため、祝日でない臨時休業では何も表示されなかった。
   → 理由を独立して表示し、「残っている予約は自動キャンセルされない」旨も併記。
3. **同時刻の予約が同じ座標に重ね描きされ、最後の1件しか見えなかった**（下敷きはクリック不可）。
   同時受付数を2以上にした店と、担当不要メニューの「未割当」レーンで発生。
   → `assignTracks`（貪欲法の段割り）を追加し、重なりを上下の段に分けて全件を可視・押下可能に。

検証: 統合22件（週の起点/営業時間との交差/定休・臨時・祝日/個別の出欠/絞り込み/退職者）＋
単体7件（段割り）→ **vitest 396 全緑**、tsc / lint / build OK。マイグレーション不要。

## 🔎 5領域スイープ + 依存のセキュリティ更新（2026-07-31）
未監査だった5領域（顧客の予約フロー / 変更・キャンセル / 通知 / 数字 / 課金・登録）を
並列監査し、対抗検証（既定 refuted=true）を通した13件を修正。
全ゲート緑（tsc / lint / **vitest 402** / build）。マイグレーション不要。

**wrong-result（結果が間違う）**
- **日時変更で予約が意図しない日に動く**: admin の変更フォームが日付/担当を変えても
  前回の枠を破棄せず、6/22 に変えたつもりで 6/15 の枠を確定できた。枠チップは時刻しか
  出さないため画面上どこにも矛盾が出ない。条件変更で枠と選択を破棄し、日付を明記。
- **日時変更が過去の売上を書き換える**: 「料金は据え置き」と明記しているのに明細を
  現在価格で作り直していた。値上げ後に日時変更するとメニュー別売上が遡って変わり、
  同じ画面の売上カードと合わなくなる。予約時の金額をスナップショットして持ち越す。
- **キャンセル後に「ご予約を承りました」が届く**: 取消時に無効化していたのは
  リマインドだけ。配信障害で滞留した確認通知が後から届き、客はキャンセル失敗と誤解する。
  その予約の PENDING 通知を全て無効化（キャンセル通知はこの後に作るので巻き添えなし）。
- **クォータのラベルと中身が違う**: 画面は「今月のオンライン予約」・ゲートは PUBLIC のみ
  なのに、集計だけが後台登録/取込を含んでいた。電話予約だけで上限に達し、
  オンライン予約が0件でも客が締め出される。両方の count を source='PUBLIC' に。
- **トライアル中がどの区分にも出ない**: Stripe status='trialing' は有料にも
  トライアルにも該当せず、経営指標の「トライアル中」が常に0だった。
- **二重契約ガードが再契約時に効かない**: ミラー列だけを見ていたため、解約→再契約で
  Webhook を取りこぼすと2本目の契約が成立し毎月二重課金される。申込直前に Stripe へ
  実在確認（listLiveSubscriptions）してから発行する。

**invisible-cause（原因が画面に出ない）**
- 予約失敗時に「店舗へお問い合わせください」と言うのに /book/{slug} に連絡先が無かった
  （QR・LIFF から来る客はホームページを経由しない）→ tel: リンクを表示。
- STAFF_UNAVAILABLE のラベルが「指名不可」。おまかせ予約でも出勤者0人なら付くため、
  誰も指名していない客に意味不明な説明が出ていた →「担当不在」（4画面が同時に直る）。
- 入力形式エラーが最終ボタンまで出ず、しかも ZodError が一律
  「入力内容を確認してください。」に潰れてどの項目か分からなかった →
  確認画面へ進む前に検証し、項目ごとに表示。ご要望欄に上限と文字数表示。
- ロック画面が完全な行き止まり: 「お支払い方法の管理」が SUBSCRIBED 分岐にしか無く、
  支払い失敗でロックされた店主はカードを直す手段が画面から消えていた。
- ロック画面の見出しが固定文「無料トライアルが終了しました」で、課金中の店主が
  支払い失敗で止まったときも同じ文が出ていた → 契約状態で出し分け。
- 課金通知（督促・トライアル終了・解約確認）の配信失敗がどの画面にも出ない:
  一覧が `booking: { shopId }` で絞っていたため bookingId=null の課金通知が全て落ちていた。
  一番気づかなければならない失敗が見えない状態だった。
- 配信失敗の件数が一覧の 50 件で頭打ちになり、実際より少なく断定表示していた →
  実数を出し、切り詰めている旨も明記。
- 予約後に店側がメニューや担当を廃止すると、日時変更が不具合にしか見えない 404 で
  終わっていた → 状況を説明し店舗への連絡を案内（勝手な振替はしない）。

**依存のセキュリティ更新**
- next-auth beta.25 → beta.32（critical 2件: 設定エラー時の fail open 等）、
  @auth/prisma-adapter 2.11.3、nodemailer 6 → 8（SMTP コマンドインジェクション・
  意図しないドメインへの送信）、sharp 0.33 → 0.35（libvips CVE 4件・商家の画像
  アップロードから到達可能）。next は 14 系最新で、残る勧告は 16.x が必要なため保留。
- lockfile を作り直すと @next/swc の linux 項目が失われ Docker の `npm ci` が壊れるため、
  対象パッケージの項目だけ差し替え（9プラットフォーム維持を確認・別環境で ci 再現確認）。

## 👁 ブラウザ実機確認で見つけた3件（2026-07-31）
ローカルで実際に起動して画面を見た結果、テストでは出なかった欠陥を発見・修正。

- **月末に予約ページを開くと、全部グレーのカレンダーだけが出る（顧客・高頻度）**
  日付の自動移動は**その月の中しか探さない**ため、31日に来た客（＝毎月必ず発生）や
  長期休業中の店では、押せる日がゼロのカレンダーが理由も次の一手も無いまま表示される。
  客は「満席なんだ」と思って離脱する。
  → 「◯年◯月は空いている日がありません。」＋「翌月の空きを見る」ボタンを追加。
  受付期間の末月なら電話番号（あれば tel: リンク）を出す。
- **日次タイムラインの勤務外が白とほぼ見分けられない** → 陰影を濃くし凡例も合わせた。
- **タイムラインの枠が中身より広く、右側に用途不明の白地が残る** → 枠を中身に合わせた。

実機で確認した動作: 日表示（勤務時間の白抜き・佐藤の14:00出勤・未割当3件の段割り）/
週表示（月曜始まり・定休日・当日ハイライト・個別出勤の反映）/ スタッフ絞り込み（1行に絞られ
他のスタッフへも切り替え可能）/ 顧客ウィザード（空月の案内→翌月へ移動→枠選択→
電話番号の形式エラーが項目直下に出て確認画面へ進めない→修正すると進める）。
コンソールエラー・ページエラーともに 0。

## 🧹 未監査5領域のスイープ + 実機クロール（2026-07-31）
顧客CRM / CSV・QR / 運営コンソール / LINE / worker を並列監査（対抗検証つき）し、
併せてローカルで全画面を実際に開いて実行時エラーを洗った。全ゲート緑（**vitest 431**）。

**実機で見つけた最重要: 入力検証の指摘が一度も画面に届いていなかった**
営業時間に同じ曜日を2行入れて保存すると、DB は正しく守られるのに画面には何も出なかった。
原因は `runAction` が ZodError を AppError として扱わず、汎用の
「処理に失敗しました。時間をおいて再度お試しください。」に落としていたこと。
スキーマ側で用意した「月曜の時間帯が重なっています（10:00〜19:00 と 10:00〜19:00）」が
**一度も店主に届いていない**うえ、案内が「時間をおいて」なので待っても直らない。
監視にも例外として送られていた（入力ミスなのに）。
→ `src/lib/zod-message.ts` を追加し、runAction と route() の両方で実文言を返す。
client 側 resolver を持たない営業時間・スタッフシフトの2フォームが特に影響を受けていた。

**顧客カルテの同定（high）**
email だけで既存カルテを探していたため、**メールを持たない予約が毎回新規カルテを作っていた**。
LIFF(LINE) 予約はメール欄自体を出さず、後台の代理登録は氏名だけで成立するため、
常連でも「来店回数 1 回 / 累計利用額 = 1回分」のまま。顧客一覧は同名だらけになり、
休眠顧客リストには同じ人が来店回数だけ並ぶ。
→ LINE → メール → 電話 の順で同定し、別チャネルで判明した連絡先は書き戻す。
キーは正規化（メール小文字化・電話は数字のみ・全角対応）して保存値と必ず一致させる。

**その他5件**
- 顧客一覧・カルテ・メモ編集がテナント単位でしか絞られておらず、店舗スコープの店員が
  他店の顧客の氏名・連絡先・来店履歴・接客メモを閲覧＆書き換えできた。
- 顧客一覧が 100 件で頭打ちなのに、その件数を「N 名の顧客」と断定表示していた（運営コンソールの
  ユーザー一覧も同様）。実数を数え、切り詰めている旨も出す。
- 送信直前の鮮度チェックが無く、キャンセル済み・来店済み・開始済みの予約にリマインドが飛んでいた
  （障害からの復帰時や失敗ジョブの手動再送で必ず起きる）。客は予約が生きていると誤解して来店する。
  キャンセル系テンプレートは除外して巻き添えを防ぐ。
- CSV の電話番号が Excel で数値化され先頭ゼロが消えていた（09012345678 → 9012345678）。
  対帳のために出した CSV から客に電話をかけられない。電話番号と予約番号をテキスト固定。

**確認したが問題なし**: CSV の数式インジェクション（=+-@ の無害化・引用符の二重化）は
実際に悪意ある氏名を投入して検証済み。全画面クロール（公開10・管理24・詳細5）で
実行時エラー・500・空白ページ・コンソールエラーはいずれも 0。

## 🧭 記録のみ（未着手・判断待ち）
- **トライアル終了後の「只読降級」案**: 現状は期限切れで `/admin` 27ページすべてが `/admin/billing` へ
  リダイレクト（`evaluateBillingAccess` → LOCKED）。一方で公開予約ページは受付を続け、
  店長のLINE/メール通知も届く。つまり **客は予約し続けるのに店主は台帳を見られない**。
  データは一切消えないが、営業中に台帳を取り上げる形になっており、転換にも逆効果の可能性。
  案: 閲覧（当日の予約・スケジュール・顧客）は残し、編集（メニュー/スタッフ/営業時間/レポート/CSV/HP）だけ
  止める＋常設バナー。**実ユーザーの反応を見てから判断する**ため今は着手しない。
